 ...
 */
public final class TwitterResultHandler extends ResultHandler {
    private static final int[] buttons = {
            R.string.button_search_from_twitter,
            R.string.button_copy_link,
            R.string.button_share,
    };

    public TwitterResultHandler(Activity activity, ParsedResult result) {
    ...
    public void handleButtonPress(int index) {
        TwitterParsedResult uriResult = (TwitterParsedResult) getResult();
        String uri = uriResult.revertRawData();
        switch (index) {
            case 0:
                String userId = uriResult.getUserId();
                if (userId != null && !AppUtils.isAppExist(App.getContext(), "com.twitter.android")) {
                    uri = String.format("https://twitter.com/%s", userId);
                }
                openURL(uri);
                break;
            case 1:
