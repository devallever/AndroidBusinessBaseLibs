 ...
 */
public final class InstagramResultHandler extends ResultHandler {
    private static final int[] buttons = {
            R.string.button_search_from_instagram,
            R.string.button_copy_link,
            R.string.button_share,
    };

    public InstagramResultHandler(Activity activity, ParsedResult result) {
    ...
    public void handleButtonPress(int index) {
        InstagramParsedResult uriResult = (InstagramParsedResult) getResult();
        String uri = uriResult.revertRawData();
        switch (index) {
            case 0:
                String userId = uriResult.getUserId();
                if (userId != null && !AppUtils.isAppExist(App.getContext(), "com.instagram.android")) {
                    uri = String.format("https://instagram.com/%s", userId);
                }
                openURL(uri);
                break;
            case 1:
