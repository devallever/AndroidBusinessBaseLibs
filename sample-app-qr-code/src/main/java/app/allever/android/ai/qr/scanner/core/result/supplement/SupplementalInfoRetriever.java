...
//        SupplementalInfoRetriever uriSafeInfoRetriever = new URISafeInfoRetriever(textView, (URIParsedResult) result, historyManager, context);
//        uriSafeInfoRetriever.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
  ...
  final void append(String itemID, String source, CharSequence[] newTexts, String linkURL) {
    ...
    boolean first = true;
    for (CharSequence newText : newTexts) {
      if (first) {
  ...
  final void append(String itemID, SpannableString spannableString) {
    newContents.add(spannableString);
    newHistories.add(new String[] {itemID, spannableString.toString()});
  }
