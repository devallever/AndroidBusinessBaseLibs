...
public final class WifiResultParser extends ResultParser {
  ...
  public WifiParsedResult parse(Result result) {
    WifiParsedResult wifiParsedResult = parse1(result);
    if (wifiParsedResult == null) {
      wifiParsedResult = parse2(result);
    }
    return wifiParsedResult;
  }

  private WifiParsedResult parse1(Result result) {
  ...
  private WifiParsedResult parse2(Result result) {
    // 解码其他应用的wifi分享内容：
    // SSID:hello Security:WPA PSW:123456
    String rawText = getMassagedText(result);
    if (!rawText.startsWith("SSID:")) {
      return null;
    }
    String ssid = matchSinglePrefixedField("SSID:", rawText, ' ', true);
    if (ssid == null || ssid.isEmpty()) {
      return null;
    }
    String type = matchSinglePrefixedField("Security:", rawText, ' ', true);
    rawText += " ";
    String psw = matchSinglePrefixedField("PSW:", rawText, ' ', true);
    return new WifiParsedResult(type, ssid, psw, false, null, null, null, null);
  }
}