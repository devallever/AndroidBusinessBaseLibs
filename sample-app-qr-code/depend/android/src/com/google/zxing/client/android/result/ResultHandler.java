...
public abstract class ResultHandler {
  ...
  protected ResultHandler(Activity activity, ParsedResult result, Result rawResult) {
    this.result = result;
  ...
  protected final boolean hasCustomProductSearch() {
  ...
  protected final void share(String contents) {
    sendEmail(null, null, null, null, contents);
  }

  protected final void shareByEmail(String contents) {
    sendEmail(null, null, null, null, contents);
  }

  protected final void sendEmail(String[] to,
                       String[] cc,
                       String[] bcc,
                       String subject,
                       String body) {
  ...
  protected final void shareBySMS(String contents) {
    sendSMSFromUri("smsto:", contents);
  }

  protected final void sendSMS(String phoneNumber, String body) {
  ...
  protected final void sendMMS(String phoneNumber, String subject, String body) {
  ...
  protected final void dialPhone(String phoneNumber) {
    launchIntent(new Intent(Intent.ACTION_DIAL, Uri.parse("tel:" + phoneNumber)));
  }

  protected final void dialPhoneFromUri(String uri) {
    launchIntent(new Intent(Intent.ACTION_DIAL, Uri.parse(uri)));
  }

  protected final void openMap(String geoURI) {
   ...
   */
  protected final void searchMap(String address) {
    launchIntent(new Intent(Intent.ACTION_VIEW, Uri.parse("geo:0,0?q=" + Uri.encode(address))));
  }

  protected final void getDirections(double latitude, double longitude) {
  ...
  protected final void webSearch(String query) {
    Intent intent = new Intent(Intent.ACTION_WEB_SEARCH);
    intent.putExtra("query", query);
    launchIntent(intent);
  }

  protected final void copy2Clipboard(String text) {
    final StringBuffer sb = new StringBuffer();
    boolean ret = ClipboardInterface.setText(text, activity, sb);
    if (ret) {
   ...
   */
  protected final void rawLaunchIntent(Intent intent) {
    if (intent != null) {
  ...
  private final String fillInCustomSearchURL(String customProductSearch, String text) {
  ...
  protected final String fillInCustomSearchURL(String text) {
    return fillInCustomSearchURL(customProductSearch, text);
  }

  protected final String fillInGoogleSearchURL(String text) {
    return fillInCustomSearchURL("https://www.google.com/search?q=%s", text);
  }

  protected static String formatPhone(String phoneData) {
