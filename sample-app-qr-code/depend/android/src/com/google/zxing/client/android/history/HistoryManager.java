...
public class HistoryManager {
    ...
    public HistoryManager(Context context, String dbName) {
    ...
    public boolean hasHistoryItems() {
        SQLiteOpenHelper helper = new DBHelper(context, this.dbName);
    ...
    public List<HistoryItem> buildHistoryItems() {
        ...
        try (SQLiteDatabase db = helper.getReadableDatabase();
            ...
            while (cursor.moveToNext()) {
                ...
                items.add(new HistoryItem(id, result, display, details));
            }
    ...
    public HistoryItem buildHistoryItem(int number) {
        ...
        try (SQLiteDatabase db = helper.getReadableDatabase();
            ...
            String details = cursor.getString(4);
            Result result = new Result(text, null, null, BarcodeFormat.valueOf(format), timestamp);
            return new HistoryItem(number, result, display, details);
        }
    }
    ...
    public void deleteHistoryItemById(int id) {
        SQLiteOpenHelper helper = new DBHelper(context, this.dbName);
        try {SQLiteDatabase db = helper.getWritableDatabase();
    ...
    public void deleteHistoryItem(List<Integer> numbers) {
        SQLiteOpenHelper helper = new DBHelper(context, this.dbName);
        try (SQLiteDatabase db = helper.getWritableDatabase();
             Cursor cursor = db.query(DBHelper.TABLE_NAME,
                     ID_COL_PROJECTION,
    ...
    public void addHistoryItem(Activity activity, Result result, ResultHandler handler) {
        // Do not save this item to the history if the preference is turned off, or the contents are
        // considered secure.
        if (activity == null && context instanceof Activity) {
            activity = (Activity) context;
        }
        if (activity != null && !activity.getIntent().getBooleanExtra(Intents.Scan.SAVE_HISTORY, true) ||
