package app.allever.android.lucky.choice.spin.finger.views
...
import app.allever.android.lucky.choice.spin.finger.circles.GroupCircle
import app.allever.android.lucky.choice.spin.finger.circles.OrderCircle
import app.allever.android.lucky.choice.spin.finger.utilities.CircleManager
import app.allever.android.lucky.choice.spin.finger.utilities.ColorManager
import app.allever.android.lucky.choice.spin.finger.utilities.Mode.GROUP
import app.allever.android.lucky.choice.spin.finger.utilities.Mode.ORDER
import app.allever.android.lucky.choice.spin.finger.utilities.Mode.SINGLE
import app.allever.android.lucky.choice.spin.finger.utilities.Number
import app.allever.android.lucky.choice.spin.finger.utilities.SoundManager
...
class Chooser(context: Context, attrs: AttributeSet?) : View(context, attrs) {
    ...
    lateinit var onButtonVisibilityChanged: (Boolean) -> Unit
    var onGameOver: () -> Unit = {}
    private val handler = Handler(Looper.getMainLooper())

    private val colorManager = ColorManager()
    private val soundManager = SoundManager(context)
    private val circles = CircleManager(context)
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (winnerChosen && mode == SINGLE) {
            ...
            circles.drawSurfaceCircles(canvas, blackRadius, scale)
        }
    ...
    private fun handleActionUp(pointerId: Int) {
        ...
        if (circles.isEmpty()) {
            val haveWinner = winnerChosen
            resetGame()
            if (haveWinner) onGameOver()
        }
    }
    ...
    private fun setButtonVisibility(visible: Boolean) {
        if (::onButtonVisibilityChanged.isInitialized) {
