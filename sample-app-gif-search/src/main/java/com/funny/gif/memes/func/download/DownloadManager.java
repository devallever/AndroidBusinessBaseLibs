    ...
    public void start(final TaskInfo taskInfo, final DownloadCallback callback, boolean forcibly) {
        ...
        if (download == null) {
            ...
            download = new OkDownloadExecutor(taskInfo, new DownloadCallback() {
                ...
                @Override
                public void onProgress(long current, long totalLength, TaskInfo taskInfo1) {
                    callback.onProgress(current, totalLength, taskInfo);
                }
                ...
                @Override
                public void onError(Exception e, TaskInfo taskInfo1) {
                    callback.onError(e, taskInfo1);
                    sDownloadExecutorMap.remove(taskInfo.getUrl());
                }
            });
