...
object AssetsUtil {
    private val mAssetsRess = WeakHashMap<String, Any>()

    fun toInputStream(path: String?): InputStream? {
        if (path != null) {
            try {
                return App.getContext().assets.open(path)
            } catch (e: IOException) {
                e.printStackTrace()
            }
        }
    ...
    private fun scaleBitmap(key: String, sourceBitmap: Bitmap, requestWith: Int, requestHeight: Int): Bitmap? {
        var scaleBitmap = mAssetsRess[key]
