...
public class GestureController implements View.OnTouchListener {
    ...
    public View getCurrentClickView() {
        return mCurrentClickView;
    }

    private View mCurrentClickView;


    private View getCurrentClickView(MotionEvent event) {
        if(targetViewGroup == null ) {
            return null;
        }
        float x = event.getRawX();
        float y = event.getRawY();
        for (int i = targetViewGroup.getChildCount()-1; i >= 0; i--) {
            View view = targetViewGroup.getChildAt(i);
            Rect rect1 = new Rect();
            view.getGlobalVisibleRect(rect1);
            if (view.getVisibility() ==View.VISIBLE && rect1.contains((int)x, (int)y)){
                return view;
            }
        }
        return null;
    ...
    public GestureController(@NonNull View view) {
        ...
        if(view instanceof ViewGroup){
            targetViewGroup = (ViewGroup)view;
        }
    ...
    private boolean temporarilyProhibitedGesture = false;
    ...
    protected boolean onTouchInternal(@NonNull View view, @NonNull MotionEvent event) {
        ...
        // 判断是不是多指模式,如果是多指只处罚放大和旋转
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                temporarilyProhibitedGesture = false;
                break;
            case MotionEvent.ACTION_UP:
            case MotionEvent.ACTION_CANCEL:
                break;
            case MotionEvent.ACTION_POINTER_DOWN:
                temporarilyProhibitedGesture = settings.isZoomEnabled() || settings.isRotationEnabled();
                break;
        ...
        if (!temporarilyProhibitedGesture) {
            result = gestureDetector.onTouchEvent(viewportEvent);
        }
//        boolean result = gestureDetector.onTouchEvent(viewportEvent);
    ...
    protected boolean onDown(@NonNull MotionEvent event) {
        ...
        mCurrentClickView = getCurrentClickView(event);
        stopFlingAnimation();
        ...
        if(mCurrentClickView != null){
            state.set(mCurrentClickView.getMatrix());
        }
        return false;
    }
    ...
    protected boolean onScroll(@NonNull MotionEvent e1, @NonNull MotionEvent e2,
        ...
        if (isScrollDetected) {
            if(mCurrentClickView != null){
                state.translateBy(mCurrentClickView,-dx, -dy);
            }else{
    ...
    protected boolean onFlingScroll(int dx, int dy) {
        ...
        if(mCurrentClickView != null){
            state.translateTo(mCurrentClickView,toX, toY);
        }else{
    ...
    protected boolean onScale(ScaleGestureDetector detector) {
        ...
        if(mCurrentClickView != null){
            pivotX = mCurrentClickView.getPivotX();
            pivotY = mCurrentClickView.getPivotY();
            state.zoomBy(mCurrentClickView,scaleFactor, pivotX, pivotY);
        }else{
    ...
    protected boolean onRotate(RotationGestureDetector detector) {
        ...
        if(mCurrentClickView != null){
            pivotX = mCurrentClickView.getPivotX();
            pivotY = mCurrentClickView.getPivotY();
            state.rotateBy(mCurrentClickView,detector.getRotationDelta(), pivotX, pivotY);
        }else{
            pivotX = detector.getFocusX();
            pivotY = detector.getFocusY();
