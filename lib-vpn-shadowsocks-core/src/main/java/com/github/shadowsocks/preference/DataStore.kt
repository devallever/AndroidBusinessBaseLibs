...
object DataStore : OnPreferenceDataStoreChangeListener {
    ...
    var serviceState = BaseService.State.Idle
        ...
        set(value) {
            field = value
//            if (_serverStateLiveData.value != value) {
            CoroutineScope(Dispatchers.IO).launch {
                _serverStateLiveData.emit(value)
            }

//            }
//            Log.d("DataStore", "update serviceState: ${value.name}")
        }
    private val _serverStateLiveData = MutableStateFlow(serviceState)
    val serverStateLiveData = _serverStateLiveData.asStateFlow()


    var mNeedReload = true
        private set
    fun updateSelectProxy(id: Long) {
        mNeedReload = profileId != id
        profileId = id
        onProxyEntityChange(id)
    }

    private var onProxyEntityChange: (Long) -> Unit = {}
    fun registerSelectedProxyChange(block: (Long) -> Unit = {}) {
        onProxyEntityChange = block
    }
