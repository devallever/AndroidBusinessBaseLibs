...
import com.github.shadowsocks.utils.CountdownManager
...
object BaseService {
    ...
    class Data internal constructor(private val service: Interface) {
        ...
        private val TOTAL_REMAIN = ShadowsSocksConfig.connectTime
        private val ADD_CONNECT_TIME = ShadowsSocksConfig.appendTime
        private val mRemainTimer by lazy {
            CountdownManager(TOTAL_REMAIN).apply {
                onFinishAction = {
                    log("remain finish")
                    binder.onTimerUpdate(0)
                    service.stopRunner(auto = true)
                    binder.onTimerUpdate(-1)
                }
                onTickAction = {
                    binder.onTimerUpdate(it)
//                    log("remain time = ${formatTimeStampToHMS(it)}, pid = ${Process.myPid()}, ${getCurrentProcessName(WhaleVpnCore.application)}")
                }
            }
        }
        ...
        fun appendConnectTime() {
            mRemainTimer.addTime(ADD_CONNECT_TIME)
        }
        ...
        fun getConnectDuration(): Long {
            return mRemainTimer.getUsedTime()
        }
    }
    ...
    class Binder(private var data: Data? = null) : IShadowsocksService.Stub(), CoroutineScope, AutoCloseable {
        ...
        override fun appendConnectTime() {
            data?.appendConnectTime()
        }

        override fun resetConnectTime() {
            data?.resetConnectTime()
        }

        override fun getConnectDuration(): Long {
            return data?.getConnectDuration()?: 0
        }
        ...
        fun onTimerUpdate(time: Long) = launch {
            broadcast { it.onTimerUpdate(time) }
        }
    ...
    interface Interface {
        ...
        fun stopRunner(restart: Boolean = false, msg: String? = null, auto: Boolean = false) {
        ...
        private fun showStopNotification() {
            try {
                val notification = NotificationCompat.Builder(Core.app, ShadowsSocksConfig.appName)
                    .setContentTitle("Connect time is over!")
                    .setTicker("Connect time is over!")
                    .setSmallIcon(ShadowsSocksConfig.notificationIcon)
                    .setCategory(NotificationCompat.CATEGORY_SERVICE)
                    .setAutoCancel(true)
                    .setPriority(NotificationCompat.PRIORITY_HIGH)
                    .setContentIntent(Core.autoStopIntent)
                    .build()
                Core.notification.notify(Core.AUTO_STOP_NOTI_ID, notification)
            } catch (e: Exception) {
                e.printStackTrace()
            }
        ...
        fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
            ...
            data.connectingJob = GlobalScope.launch(Dispatchers.Main) {
                try {
                    ...
                    stopRunner(false, "${"Failed to connect the remote server"}: ${exc.readableMessage}")
                } finally {
