...
import com.hd.calculator.app.databinding.ActivitySplitPaymentBinding;
...
import com.hd.calculator.app.ui.adapter.ItemClickListener;
 ...
 * 拆分付款
 */
public class SplitPaymentActivity extends BaseActivity<ActivitySplitPaymentBinding> {
    //curret list
    ...
    @Override
    protected ActivitySplitPaymentBinding getViewBinding() {
        return ActivitySplitPaymentBinding.inflate(getLayoutInflater());
    }
        ...
        ThreadUtils.runOnIoThreadDelayed(() -> {
            ...
            for (OrderDishesRecordEntity dishesRecord : fromOrderList.getDishesList()) {
                ...
                if (fromId2DishesMap.containsKey(dishesRecord.getId())) {
                    ...
                    // 添加删除订单菜品日志
                    EventUtils.logDeleteOrderEvent(mOrderId, "分账时删除转移的菜品");
                    //删除记录
            ...
            TableManager.getIns().postOrderRecord(mTableCode, ActionType.SPLIT_BILL, false, () -> {
                if (mFromList.isEmpty()) {
                    // 添加删除订单日志
                    EventUtils.logDeleteOrderEvent(mOrderId, "分账后原订单菜品为空删除订单");
                    //删除订单
