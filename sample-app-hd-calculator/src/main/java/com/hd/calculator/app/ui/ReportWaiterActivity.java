...
import com.hd.calculator.app.databinding.HdcActivityReportDetailBinding;
 ...
 * 跑堂日报表
 */
public class ReportWaiterActivity extends BaseActivity<HdcActivityReportDetailBinding> {
    ...
    @Override
    protected HdcActivityReportDetailBinding getViewBinding() {
        return HdcActivityReportDetailBinding.inflate(getLayoutInflater());
    }
    ...
    private void initReportData() {
        ThreadUtils.runOnIoThreadDelayed(() -> {
            ...
            //收银总额: 点餐菜牌金额+小费金额
            mList.add(new ReportDetailItem(getString(R.string.hdc_hdc_report_label_total_income), MoneyUtils.formatMoney(payTotal)));

            //刷卡单数:没有被取消或者恢复台的 刷卡买单的账单数
            mList.add(new ReportDetailItem(getString(R.string.hdc_report_label_total_card_order_count), cardBillCount+""));
            //刷卡买单总额:没有被取消或者恢复台的 刷卡买单的点餐菜牌金额+小费金额
            mList.add(new ReportDetailItem(getString(R.string.hdc_report_label_total_card_order_amount), MoneyUtils.formatMoney(cardPayTotal)));
            //刷卡小费总额:没有被取消或者恢复台的 刷卡买单的
            mList.add(new ReportDetailItem(getString(R.string.hdc_report_label_total_card_order_tips), MoneyUtils.formatMoney(cardTipsTotal)));
            //现金单数:没有被取消或者恢复台的 现金买单的帐单数
            mList.add(new ReportDetailItem(getString(R.string.hdc_report_label_total_cash_order_count), cashBillCount + ""));
            //现金买单总额:没有被取消或者恢复台的 现金买单的点餐金额+小费金额
            mList.add(new ReportDetailItem(getString(R.string.hdc_report_label_total_cash_order_amount), MoneyUtils.formatMoney(cashPayTotal)));

            float value = cashPayTotal - cardTipsTotal;
            //需交现金数(=现金买单总额-刷卡小费总额
            mList.add(new ReportDetailItem(getString(R.string.hdc_report_label_need_cash_amount), MoneyUtils.formatMoney(value)));
