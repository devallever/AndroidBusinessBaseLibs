...
class ReceiveCoinViewModel : ViewModel() {

    var residueInfiniteTime = MutableLiveData<Long>()  //剩余的无限制时间
    var gameCount = MutableLiveData<Int>()  //剩余的接金币次数
    ...
    init {
        ...
        residueInfiniteTime.observeForever {
            SpUtil.put(SpKey.RECEIVE_COIN_GAME_COUNT_INFINITE_TIME, it)
            isInfiniteTime.value  = residueInfiniteTime.value!! > 0
        }
        gameCount.observeForever {
            SpUtil.put(SpKey.RECEIVE_COIN_GAME_COUNT, it)
            if (it < 3 && startCountDownTimer) {
                startCountDownTimer = false
                if (App.DEBUG){
                    LogUtil.receiveCoin("接金币次数不足三次，当前次数：$it 开始获取倒计时，剩余获取时间：${countDownTimer.value}")
                }
                getCountDownTimer(
                    ...
                    },
                    {
                        ...
                        if (App.DEBUG) {
                            LogUtil.receiveCoin("倒计时结束获取接金币次数，当前次数${gameCount.value}")
                        }
                    }).start()
            }
        }
        countDownTimer.observeForever {
            SpUtil.put(SpKey.RECEIVE_COIN_GAME_ADD_COUNT_TIME, it)
        }

        isInfiniteTime.observeForever {
            if (App.DEBUG) {
                LogUtil.receiveCoin("接金币无限次数？$it")
            }
            if (it && startInfiniteCountDownTimer) {
                ...
                getCountDownTimer(
                    ...
                    10000L,
                    {
                        ...
                        if (App.DEBUG) {
                            LogUtil.receiveCoin("接金币无限次数倒计时，当前剩余无限制时间：${residueInfiniteTime.value}")
                        }
                    },
                    {
                        ...
                        if (App.DEBUG) {
                            LogUtil.receiveCoin("接金币无限次数时间耗尽,isInfiniteTime: ${isInfiniteTime.value},residueInfiniteTime: ${residueInfiniteTime.value}")
                        }
                    }).start()
            }
        }
    ...
    fun updateResidueInfiniteTime() {
        val residueTime = SpUtil.get(SpKey.RECEIVE_COIN_GAME_COUNT_INFINITE_TIME, 0L)
        val lastAppQuitTime = SpUtil.get(SpKey.APP_QUIT_TIME, 0L)
        val quitTime = System.currentTimeMillis() - lastAppQuitTime
        if (App.DEBUG) {
            LogUtil.receiveCoin("接金币无限时间更新，离线时间：$quitTime 剩余无限时间：$residueTime")
        }
        residueInfiniteTime.value = if (quitTime >= residueTime) {
            if (App.DEBUG) {
                LogUtil.receiveCoin("离线时间大于剩余无限时间，无限时间清零")
            }
            0L
        } else {
            if (App.DEBUG) {
                LogUtil.receiveCoin("离线时间小于剩余无限时间，剩余无限时间：${residueTime - quitTime}")
            }
    ...
    fun updateGameCount() {
        var count = SpUtil.get(SpKey.RECEIVE_COIN_GAME_COUNT, 3)
        val lastAppQuitTime = SpUtil.get(SpKey.APP_QUIT_TIME, 0L)
        var quitTime = System.currentTimeMillis() - lastAppQuitTime
        var residueAddTime = SpUtil.get(SpKey.RECEIVE_COIN_GAME_ADD_COUNT_TIME, addGameCountTime)
        if (App.DEBUG) {
            LogUtil.receiveCoin("接金币次数更新，离线时间：$quitTime 游戏次数：$count 剩余获取时间：${residueAddTime}" )
        }
        if (count < 3) {
            if (App.DEBUG) {
                LogUtil.receiveCoin("接金币次数小于3，计算离线补偿")
            }
            if (quitTime >= residueAddTime) {
                count++
                quitTime -= residueAddTime
                if (App.DEBUG) {
                    LogUtil.receiveCoin("离线时间大于剩余获取时间，接金币次数+1：$count 剩余离线时间 = 离线时间-剩余获取时间：${quitTime}")
                }
                while (quitTime > 0 && count < 3) {
                    if (quitTime >= addGameCountTime) {
                        ...
                        if (App.DEBUG) {
                            LogUtil.receiveCoin("可用离线时间大于一次获取时间 并且 接金币次数小于3，接金币次数+1：$count  获取一次机会所需时间：${addGameCountTime}  剩余可用离线时间 = 离线时间-获取一次机会所需时间：${quitTime}")
                        }
                    } else {
                        residueAddTime = addGameCountTime - quitTime
                        quitTime = 0
                        if (App.DEBUG) {
                            LogUtil.receiveCoin("可用离线时间小于一次获取时间 下一次获取时间 = 获取一次机会所需时间（${addGameCountTime}）- 可用离线时间（${quitTime}）：${residueAddTime}")
                        }
                    }
                }
                ...
                if (App.DEBUG) {
                    LogUtil.receiveCoin("离线时间小于剩余获取时间，剩余获取时间 = 剩余获取时间（${residueAddTime}）- 离线时间（${quitTime}）：${residueAddTime}")
                }
            }
        }
