...
object StepManager {
    ...
    fun startTodayStepService(application: Application?) {
        try {
            val intent = Intent(application, StepService::class.java)
...
//            对开发者处理建议：在服务启动的地方进行try catch防止崩溃即可（也是“1元夺宝”APP目前的处理方式）
    ...
    fun bindService(activity: Activity, conn: ServiceConnection?): Boolean {
        try {
            val intent = Intent(activity, StepService::class.java)
