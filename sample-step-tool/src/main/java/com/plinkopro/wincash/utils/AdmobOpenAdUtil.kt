...
import com.plinkopro.wincash.ui.activity.HotLaunchActivity
...
object AdmobOpenAdUtil {
    ...
    fun onAppBackgrounded(activity: Activity) {
        ...
        if (BuildConfig.LOG_OUTPUT) Log.d("launch_tag","app 进入后台 记录时间")

        //判断有没有缓存
        val isShowSuccess = AdManager.isCanShowAdmobOpenAd()
        if (!isShowSuccess) {
            AdManager.loadAdmobAppOpenAd(activity, Constance.ADMOB_SPLASH_ID)
        }
    ...
    fun onAppForegrounded(activity: Activity) {
        ...
        if (activityName == LaunchActivity::class.java.name) {
            if (BuildConfig. LOG_OUTPUT) Log.d("launch_tag","app 返回前台 还在开屏页")
            return
        }
        if (!activity.isSelfClass()) {
            if (BuildConfig. LOG_OUTPUT) Log.d("launch_tag","app 返回前台 正在播放广告 ： $activityName")
            return
        }

        val currentRealTime = SystemClock.elapsedRealtime()
        val backgroundTime = currentRealTime - inBackgroundTime
        if (BuildConfig. LOG_OUTPUT) Log.d("launch_tag", "app 进入后台时间 ： ${backgroundTime / 1000}秒")

        val holdTime = currentRealTime - showTime
        if (BuildConfig. LOG_OUTPUT) Log.d("launch_tag", "app 距离上一次展示时间 ： ${holdTime / 1000}秒")
