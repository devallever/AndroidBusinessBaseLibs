...
import com.plinkopro.wincash.ui.activity.STLaunchActivity
...
//        }
    ...
    fun onAppForegrounded(activity: Activity) {
        ...
        if (activityName == STLaunchActivity::class.java.name) {
            if (App.DEBUG) Log.d("launch_tag","app 返回前台 还在开屏页")
            return
        }
        if (!activity.isSelfClass()) {
            if (App.DEBUG) Log.d("launch_tag","app 返回前台 正在播放广告 ： $activityName")
            return
        }

        val currentRealTime = SystemClock.elapsedRealtime()
        val backgroundTime = currentRealTime - inBackgroundTime
        if (App.DEBUG) Log.d("launch_tag", "app 进入后台时间 ： ${backgroundTime / 1000}秒")

        val holdTime = currentRealTime - showTime
        if (App.DEBUG) Log.d("launch_tag", "app 距离上一次展示时间 ： ${holdTime / 1000}秒")
