...
class CustomTimer {
    ...
    private val timerRunnable = object : Runnable {
        override fun run() {
            ...
            if (App.DEBUG) {
                log("CustomTimer: $totalElapsedTime")
            }
     ...
     */
    fun setInterval(intervalMs: Long): CustomTimer {
        if (intervalMs <= 0) {
            throw kotlin.IllegalArgumentException("Interval must be greater than 0")
        }
     ...
     */
    fun setMaxRunningTime(maxTimeMs: Long?): CustomTimer {
        if (maxTimeMs != null && maxTimeMs <= 0) {
            throw kotlin.IllegalArgumentException("Max time must be greater than 0")
        }
     ...
     */
    fun setTickCallback(callback: (elapsedTime: Long) -> Unit): CustomTimer {
     ...
     */
    fun setFinishCallback(callback: () -> Unit): CustomTimer {
