...
fun Activity?.isSelfClass(): Boolean =
    this?.javaClass?.name?.startsWith("com.example.charge") == true
...
fun readJsonFromRaw(resourceId: Int): String {
    return try {
        val inputStream = ChargeApp.instance.resources.openRawResource(resourceId)
