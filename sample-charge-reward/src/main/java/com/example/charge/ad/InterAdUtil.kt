...
import com.example.charge.utils.RandomUtils
...
object InterAdUtil {
    ...
    fun showAd(): Boolean {
        ...
        return if (isshow) {
            val show = Random.Default.nextBoolean()
            if (App.DEBUG) {
    ...
    fun isMatchLogic(logic: Int): Boolean {
        val result = FpManger.chargeConfig.interLogic == logic
        if (App.DEBUG) {
            LogUtil.showInterAd("插屏逻辑：${FpManger.chargeConfig.interLogic}")
        }
        return result
    }

    fun isProbabilityHit(): Boolean {
        val probability = RandomUtils.isProbabilityHit(FpManger.chargeConfig.gameEndInterAdProbability)
        if (App.DEBUG) {
            if ( probability) {
                LogUtil.showInterAd("命中插屏概率：${FpManger.chargeConfig.gameEndInterAdProbability}")
            } else {
                LogUtil.showInterAd("未命中插屏概率：${FpManger.chargeConfig.gameEndInterAdProbability}")
            }
        }
        return probability
    }
