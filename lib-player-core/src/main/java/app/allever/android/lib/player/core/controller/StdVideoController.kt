package app.allever.android.lib.player.core.controller
...
import app.allever.android.lib.player.core.R
import app.allever.android.lib.player.core.VideoScaleMode
import app.allever.android.lib.player.core.databinding.StdUiControllerBinding
...
class StdVideoController(val context: Context) : IVideoUiController {
    ...
    override fun onScaleModeChanged(mode: VideoScaleMode) {
        val iconRes = when (mode) {
            VideoScaleMode.FIT_CENTER -> R.drawable.lpc_ic_scale_fit
            VideoScaleMode.CROP_CENTER -> R.drawable.lpc_ic_scale_crop
            VideoScaleMode.STRETCH -> R.drawable.lpc_ic_scale_stretch
        }
    ...
    override fun onLoopModeChanged(mode: LoopMode) {
        val iconRes = when (mode) {
            LoopMode.NONE -> R.drawable.lpc_ic_loop_none
            LoopMode.SINGLE -> R.drawable.lpc_ic_loop_single
            LoopMode.ALL -> R.drawable.lpc_ic_loop_all
        }
    ...
    override fun onFullscreenChanged(isFullscreen: Boolean) {
        val iconRes = if (isFullscreen) {
            R.drawable.lpc_ic_fullscreen_exit  // 横屏时显示退出全屏图标
        } else {
            R.drawable.lpc_ic_fullscreen_enter  // 竖屏时显示进入全屏图标
        }
    ...
    override fun onGestureVolumeProgressChanged(volume: Float, progress: Int) {
        ...
        // 根据音量更新图标
        val iconRes = when {
            volume <= 0f -> R.drawable.lpc_ic_volume_mute
            volume < 0.33f -> R.drawable.lpc_ic_volume_low
            volume < 0.66f -> R.drawable.lpc_ic_volume_medium
            else -> R.drawable.lpc_ic_volume_up
        }
    ...
    override fun onPlayStateChanged(isPlaying: Boolean) {
        binding.ivVPPlayPause.setImageResource(if (isPlaying) R.drawable.lpc_ic_sample_video_player_view_pause else R.drawable.lpc_ic_sample_video_player_view_play)
    }
