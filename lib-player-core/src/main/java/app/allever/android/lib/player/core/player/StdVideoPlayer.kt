package app.allever.android.lib.player.core.player
...
import app.allever.android.lib.player.core.controller.StdVideoController
import app.allever.android.lib.player.core.databinding.StdVideoPlayerBinding
...
open class StdVideoPlayer @JvmOverloads constructor(
    ...
    private val SCALE_MODE_LIST = arrayOf(
        VideoScaleMode.FIT_CENTER, VideoScaleMode.CROP_CENTER, VideoScaleMode.STRETCH
    )

    /** 循环模式图标映射 */
    private val LOOP_MODE_LIST = arrayOf(
        LoopMode.NONE, LoopMode.SINGLE, LoopMode.ALL
    )
    ...
    override fun onFinishInflate() {
        ...
        uiController = bindUiController()
        binding.controllerContainer.removeAllViews()
        binding.controllerContainer.addView(uiController?.getRootView())
    ...
    private fun extractAssetTitle(path: String): String {
        ...
        // 清理文件名：去除扩展名，美化显示
        return fileName.substringBeforeLast(".")  // 去除扩展名
            .replace("_", " ")       // 下划线转空格
            .replace("-", " ")       // 连字符转空格
            .trim().ifEmpty { "视频" }      // 如果为空，使用默认标题
    }
    ...
    fun switchEngine(targetType: String) {
        ...
        appendLog(
            "开始切换引擎: ${currentEngineType} -> $targetType" + (if (wasPlaying) " (正在播放，位置=${
                formatTime(
                    savedPosition
                )
            })" else "")
        ...
        videoPlayer = VideoPlayer(
            engine = newEngine, render = currentRender
        ).apply {
    ...
    fun switchRender(renderName: String) {
        ...
        appendLog(
            "开始切换渲染器: $currentRenderName -> $renderName" + (if (wasPlaying) " (正在播放，位置=${
                formatTime(
                    savedPosition
                )
            })" else "")
    ...
    private fun initPlayer() {
        ...
        videoPlayer = VideoPlayer(
            engine = MediaPlayerEngine(), render = SurfaceViewRender()
        ).apply {
            attach(binding.renderContainer)
            setListener(playerListener)
            retryCount = 3
    ...
    private fun initClickListener() {
        // 返回按钮
        uiController?.getBackView()?.setOnClickListener {
        ...
        // 循环模式按钮
        uiController?.getLoopModeView()?.setOnClickListener {
            switchLoopMode()
        }

        uiController?.getSpeedView()?.setOnClickListener {
            switchSpeed()
        }

        uiController?.getScaleModeView()?.setOnClickListener {
            switchScaleMode()
        }

        uiController?.getPlayPauseView()?.setOnClickListener {
        ...
        val seekBarChangeListener = object : SeekBar.OnSeekBarChangeListener {
            ...
            override fun onStopTrackingTouch(seekBar: SeekBar?) {
                if (videoPlayer.duration > 0 && seekBar != null) {
                    val position =
                        (seekBar.progress.toFloat() / 100 * videoPlayer.duration).toLong()
                    videoPlayer.seekTo(position)
                }
                uiController?.getSeekBarView()?.post { isUserSeeking = false }
            }
        }

        uiController?.getSeekBarView()?.setOnSeekBarChangeListener(seekBarChangeListener)
    }
    ...
    private fun handleSeekGesture(deltaX: Float, containerWidth: Float) {
        ...
        gestureTargetPosition =
            (gestureTargetPosition + deltaTime).coerceIn(0, videoPlayer.duration)
