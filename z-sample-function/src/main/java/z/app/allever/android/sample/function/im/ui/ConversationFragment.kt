package z.app.allever.android.sample.function.im.ui
...
import z.app.allever.android.sample.function.databinding.FragmentConversationBinding
import z.app.allever.android.sample.function.im.ui.adapter.ExpandItem
import z.app.allever.android.sample.function.im.ui.adapter.ScrollBoundaryDeciderAdapter
import z.app.allever.android.sample.function.im.ui.widget.InputBar
...
class ConversationFragment :
    ...
    private val imageLauncher = MediaPickerCore.registerPickerLauncher(this) {
        it.map {
            mViewModel.sendMediaMessage(it, MediaType.Type.IMAGE)
        }
        mBinding.recyclerView.scrollToPosition(0)
    }

    private val videoLauncher = MediaPickerCore.registerPickerLauncher(this) {
        it.map {
            mViewModel.sendMediaMessage(it, MediaType.Type.VIDEO)
        }
        mBinding.recyclerView.scrollToPosition(0)
    }
    ...
    private fun handleExpFunClick(expandItem: ExpandItem) {
        when (expandItem.type) {
            ExpandItem.TYPE_IMAGE -> {
                MediaPickerCore.launchImage(imageLauncher, 9)
            }
            ExpandItem.TYPE_VIDEO -> {
                MediaPickerCore.launchVideo(videoLauncher, 9)
            }
        ...
        inputBarDialog = InputBarDialog(
            ...
            object : InputBar.InputBarListener {
                ...
                override fun onClickAdd() {
                    ...
                    builder.request { allGranted, deniedList ->
                        if (allGranted) {
                            toast("同意权限")
